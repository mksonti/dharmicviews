...
import { Search, PlayCircle, Filter, Calendar, Tag, User } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import Link from 'next/link';
import Image from 'next/image';
import type { VideoData } from '@/lib/videos';

type SortOption = 'title-asc' | 'title-desc' | 'date-newest' | 'date-oldest' | 'publisher' | 'channel';
...
export default function VideosClient({ initialVideos }: { initialVideos: VideoData[] }) {
  ...
  const filteredAndSortedVideos = useMemo(() => {
    ...
    return result;
  }, [initialVideos, searchQuery, selectedTag, sortBy]);
          ...
          <div className="flex gap-2">
            <div className="relative flex-1 sm:flex-none">
              <select
                ...
                className="w-full sm:w-auto appearance-none pl-4 pr-10 py-2 bg-white border border-stone-200 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none text-stone-700 text-sm cursor-pointer"
              >
              ...
              <Filter className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 w-4 h-4 pointer-events-none" />
            </div>
          </div>
        </div>
      </div>
            ...
            {filteredAndSortedVideos.map((video) => (
              ...
              >
                <Link href={`/videos/${video.videoId}`} className="group block h-full">
                  <div className="bg-white rounded-2xl border border-orange-100 overflow-hidden shadow-sm hover:shadow-md transition-all h-full flex flex-col relative">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <h2 className="text-lg font-bold text-stone-900 mb-2 line-clamp-2 group-hover:text-orange-600 transition-colors">
                        {video.title}
                      </h2>
